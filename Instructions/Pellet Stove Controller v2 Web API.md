...
You may notice that under heavy API use, the uptime display on the screen appears to skip seconds. This is absolutely normal and doesn't affect the actual operation of the stove. The top auger timer is an interrupt driven low speed PWM, nothing in the main loop() can affect it. The screen update and API handler run in the main loop() and heavy API use will cause lags in screen updates.
